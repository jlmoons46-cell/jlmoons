...
### `recovery_requests`
Stores the technical intake data submitted by users for forensic assessment. Quoted identifiers are used to maintain camelCase compatibility with the application code.
...
### `assets` Bucket
Used for platform images. Ensure the bucket is **Public**.

```sql
-- Required Storage Policies for Admin Uploads
